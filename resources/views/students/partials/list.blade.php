<div x-show="tab === 'list'" x-cloak
    ...
    <div class="fixed top-4 right-4 md:top-6 md:right-6 z-[9999] flex flex-col gap-3 pointer-events-none max-w-[calc(100vw-2rem)] md:max-w-sm">
        ...
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
            ...
            class="pointer-events-auto flex items-center gap-3 bg-white border border-emerald-200 shadow-xl px-4 py-3 rounded-2xl">
            <div class="w-8 h-8 bg-emerald-500 rounded-xl flex items-center justify-center text-white shrink-0"><i class="bi bi-check2"></i></div>
        ...
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-8"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-end="opacity-0 translate-x-8"
            class="pointer-events-auto flex items-center gap-3 bg-white border border-rose-200 shadow-xl px-4 py-3 rounded-2xl">
            <div class="w-8 h-8 bg-rose-500 rounded-xl flex items-center justify-center text-white shrink-0"><i class="bi bi-exclamation-lg"></i></div>
            <div class="flex-1 min-w-0">
    ...
    {{-- Quick Stats — dihitung PHP di server, akurat setiap page load --}}
    @php
        $countPending  = $aspirasi->where('status', 'pending')->count();
        $countProses   = $aspirasi->where('status', 'proses')->count();
        $countSelesai  = $aspirasi->where('status', 'selesai')->count();
    @endphp
    <div class="grid grid-cols-3 gap-2 md:gap-3 mb-5 md:mb-7">
        <div class="bg-white border border-slate-100 rounded-2xl p-3 md:p-4 flex items-center gap-2 md:gap-3 shadow-sm">
            <div class="w-7 h-7 md:w-8 md:h-8 bg-amber-100 text-amber-600 rounded-xl flex items-center justify-center text-xs md:text-sm shrink-0">
                <i class="bi bi-clock-fill"></i>
            </div>
            <div>
                <p class="text-[8px] md:text-[9px] text-slate-400 font-black uppercase tracking-wider leading-none">Pending</p>
                <p class="text-base md:text-lg font-black text-slate-800 leading-none mt-0.5">{{ sprintf('%02d', $countPending) }}</p>
            </div>
        </div>
        <div class="bg-white border border-slate-100 rounded-2xl p-3 md:p-4 flex items-center gap-2 md:gap-3 shadow-sm">
            <div class="w-7 h-7 md:w-8 md:h-8 bg-sky-100 text-sky-600 rounded-xl flex items-center justify-center text-xs md:text-sm shrink-0">
                <i class="bi bi-hourglass-split"></i>
            </div>
            <div>
                <p class="text-[8px] md:text-[9px] text-slate-400 font-black uppercase tracking-wider leading-none">Diproses</p>
                <p class="text-base md:text-lg font-black text-slate-800 leading-none mt-0.5">{{ sprintf('%02d', $countProses) }}</p>
            </div>
        </div>
        <div class="bg-white border border-slate-100 rounded-2xl p-3 md:p-4 flex items-center gap-2 md:gap-3 shadow-sm">
            <div class="w-7 h-7 md:w-8 md:h-8 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center text-xs md:text-sm shrink-0">
                <i class="bi bi-check2-circle"></i>
            </div>
            <div>
                <p class="text-[8px] md:text-[9px] text-slate-400 font-black uppercase tracking-wider leading-none">Selesai</p>
                <p class="text-base md:text-lg font-black text-slate-800 leading-none mt-0.5">{{ sprintf('%02d', $countSelesai) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-3 md:space-y-4">
        ...
        @php
            $sc = [
                'pending' => ['bar'=>'bg-amber-400','badge'=>'bg-amber-50 text-amber-700 border-amber-200','label'=>'Pending','icon'=>'bi-clock-fill','glow'=>'hover:border-amber-200'],
                'proses'  => ['bar'=>'bg-sky-500',  'badge'=>'bg-sky-50 text-sky-700 border-sky-200',     'label'=>'Diproses','icon'=>'bi-hourglass-split','glow'=>'hover:border-sky-200'],
                'selesai' => ['bar'=>'bg-emerald-500','badge'=>'bg-emerald-50 text-emerald-700 border-emerald-200','label'=>'Selesai','icon'=>'bi-check2-circle','glow'=>'hover:border-emerald-200'],
            ][$list->status] ?? ['bar'=>'bg-amber-400','badge'=>'bg-amber-50 text-amber-700 border-amber-200','label'=>'Pending','icon'=>'bi-clock-fill','glow'=>'hover:border-amber-200'];
        @endphp

        <div x-data="{ openDetail: false, openEdit: false, openDelete: false }"
            class="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 {{ $sc['glow'] }}">
            <div class="h-1 {{ $sc['bar'] }}"></div>
            <div class="p-4 md:p-5">
                <div class="flex items-start gap-3 md:gap-4">

                    {{-- Index --}}
                    <div class="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0">
                    ...
                    <div class="flex-1 min-w-0 cursor-pointer" @click="openDetail = !openDetail">
                        <div class="flex flex-wrap items-center gap-1.5 mb-1.5">
                            <span class="inline-flex items-center gap-1 text-[9px] md:text-[10px] font-black px-2 py-0.5 rounded-lg border {{ $sc['badge'] }}">
                                <i class="bi {{ $sc['icon'] }}"></i> {{ $sc['label'] }}
                            </span>
                            <span class="text-[9px] md:text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100">
                                {{ $list->category->name ?? 'Umum' }}
                            </span>
                            <span class="text-[9px] text-slate-300 font-medium hidden sm:inline">{{ $list->created_at->format('d M Y') }}</span>
                        </div>
                    ...
                    <div class="flex items-center gap-1.5 shrink-0">
                        ...
                        @else
                        <div class="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center text-slate-300 bg-slate-50 rounded-xl border border-slate-100 cursor-not-allowed" title="Tidak bisa diubah">
                ...
                {{-- Detail expand — tanpa x-collapse, pakai x-show + transition biasa --}}
                <div x-show="openDetail" x-cloak class="mt-3 md:mt-4 space-y-2 md:space-y-3"
                    ...
                    <div class="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-3 md:p-4 border border-emerald-100 relative overflow-hidden">
                    ...
                    <div class="bg-slate-50 rounded-xl p-3 border border-dashed border-slate-200 flex items-center gap-2">
                        <i class="bi bi-three-dots text-slate-300"></i>
                        <p class="text-[10px] text-slate-400 font-medium">Menunggu tanggapan admin...</p>
                    </div>
                    @endif
                </div>
            </div>
                ...
                <div x-show="openEdit" x-cloak
                    ...
                    <div @click.away="openEdit = false"
                        ...
                        <div class="bg-gradient-to-r from-slate-800 to-slate-900 px-6 py-4 flex items-center justify-between">
                            ...
                            <button @click="openEdit = false" class="w-8 h-8 bg-white/10 hover:bg-white/20 rounded-xl flex items-center justify-center text-white/70 transition-all">
                                <i class="bi bi-x-lg text-sm"></i>
                            </button>
                        </div>
                        <form action="{{ route('aspirasi.update', $list->id) }}" method="POST" class="p-6 space-y-4">
                            @csrf @method('PUT')
                            <div>
                                ...
                                <input type="text" name="title" value="{{ $list->title }}" required
                                    class="w-full border-2 border-slate-100 bg-slate-50 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:border-blue-500 focus:bg-white outline-none transition-all">
                            </div>
                            <div>
                                <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Isi Aspirasi</label>
                                <textarea name="content" rows="4" required
                                    class="w-full border-2 border-slate-100 bg-slate-50 rounded-xl px-4 py-3 text-sm font-medium text-slate-600 focus:border-blue-500 focus:bg-white outline-none transition-all resize-none">{{ $list->content }}</textarea>
                            </div>
                            <div class="flex gap-3">
                                <button type="button" @click="openEdit = false"
                                    class="flex-1 py-3 bg-slate-100 text-slate-500 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-200 transition-all">Batal</button>
                                <button type="submit"
                                    class="flex-[2] py-3 bg-blue-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg shadow-blue-200 active:scale-95 transition-all">
                    ...
                    <div @click.away="openDelete = false"
                        ...
                        <div class="p-6 text-center">
                            <div class="w-14 h-14 bg-rose-50 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-rose-100">
                                <i class="bi bi-trash3-fill text-xl text-rose-500"></i>
                            </div>
                            <h3 class="font-black text-slate-900 text-lg mb-2">Hapus Aspirasi?</h3>
                        ...
                        <div class="px-6 pb-6 flex gap-3">
                            ...
                            <form action="{{ route('aspirasi.destroy', $list->id) }}" method="POST" class="flex-1">
                                @csrf @method('DELETE')
                                <button type="submit"
                                    class="w-full py-3 bg-rose-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-rose-700 shadow-lg shadow-rose-200 active:scale-95 transition-all">Hapus!</button>
                            </form>
                        </div>
                    </div>
                </div>
        ...
        @empty
        <div class="text-center py-16 bg-white border-2 border-dashed border-slate-200 rounded-2xl">
